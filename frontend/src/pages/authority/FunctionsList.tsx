...
import { allFunctions, FunctionEntity } from "../../apis/actions/functions.action";

const FunctionsList = () => {
    const [searchParam] = useSearchParams();
    const dispatch = useAppDispatch();
    const navigate = useNavigate();
    const { loading, functions, pagination, error, success, message } = useAppSelector((state) => state.functions);
    const [keyword, setKeyword] = useState(searchParam.get("keyword") ?? undefined);
    const [libraryDelete, setLibraryDelete] = useState<FunctionEntity | null>(null);
    ...
    useEffect(() => {
        ...
        dispatch(allFunctions({ keyword: searchParam.get("keyword"), page: searchParam.get("page") ? parseInt(searchParam.get("page")!) : 1, view: searchParam.get("view") ? parseInt(searchParam.get("view")!) : 10 }));
    ...
    const toggleModalDelete = (item: FunctionEntity) => {
            ...
            {loading ?
                ...
                <div>
                    ...
                    <div className="box-handle mb-3">
                        <Link to="/functions/create" className="btn-fill rounded">Create</Link>
                                            ...
                                            <tr key={item._id}>
                                                ...
                                                <td className="align-middle">
                                                    <div className="btn-group d-flex gap-2 align-items-center">
                                                        <Link className="btn-icon" to={`/functions/view/${item._id}`} data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="View">
                                                            <div className="icon">
                                                                <RiInformation2Line />
                                                            </div>
                                                        </Link>
                                                        <Link className="btn-icon" to={`/functions/edit/${item._id}`} data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Edit">
                                                            <div className="icon">
                                                                <RiPencilLine />
                                                            </div>
                                                        </Link>
