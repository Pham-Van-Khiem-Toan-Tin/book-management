...
import { allSubFunctions, SubFunction } from "../../apis/actions/subfunction.action";
import { reset, resetError } from "../../apis/slices/subfunction.slice";

const SubFunctionsList = () => {
    const [searchParam] = useSearchParams();
    const dispatch = useAppDispatch();
    const navigate = useNavigate();
    const { loading, subfunctions, pagination, error, success, message } = useAppSelector((state) => state.subFunction);
    const [keyword, setKeyword] = useState(searchParam.get("keyword") ?? undefined);
    const [libraryDelete, setLibraryDelete] = useState<SubFunction | null>(null);
    ...
    useEffect(() => {
        ...
        dispatch(allSubFunctions({ keyword: searchParam.get("keyword"), page: searchParam.get("page") ? parseInt(searchParam.get("page")!) : 1, view: searchParam.get("view") ? parseInt(searchParam.get("view")!) : 10 }));
    ...
    const toggleModalDelete = (item: SubFunction) => {
            ...
            {loading ?
                ...
                <div>
                    ...
                    <div className="box-handle mb-3">
                        <Link to="/subfunctions/create" className="btn-fill rounded">Create</Link>
                                            ...
                                            <tr key={item._id}>
                                                ...
                                                <td className="align-middle">
                                                    <div className="btn-group d-flex gap-2 align-items-center">
                                                        <Link className="btn-icon" to={`/subfunctions/view/${item._id}`} data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="View">
                                                            <div className="icon">
                                                                <RiInformation2Line />
                                                            </div>
                                                        </Link>
                                                        <Link className="btn-icon" to={`/subfunctions/edit/${item._id}`} data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Edit">
                                                            <div className="icon">
                                                                <RiPencilLine />
                                                            </div>
                                                        </Link>
                                ...
                                <span>Display
                                    <div className="select-record">
                                        <Select
                                            ...
                                            isDisabled={subfunctions.length == 0}
...
export default SubFunctionsList