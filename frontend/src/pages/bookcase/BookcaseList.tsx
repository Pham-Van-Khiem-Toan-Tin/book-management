...
import { allBookcase, Bookcase, deleteBookcase } from "../../apis/actions/bookcase.action";
...
const BookcaseList = () => {
    ...
    const { loading, message, bookcases, error, pagination, success } = useAppSelector((state) => state.bookcase);
    const [keyword, setKeyword] = useState(searchParam.get("keyword") ?? undefined);
    const [categoryDelete, setCategoryDelete] = useState<Bookcase | null>(null);
    ...
    useEffect(() => {
        ...
        dispatch(allBookcase({ keyword: searchParam.get("keyword"), view: parseInt(searchParam.get("view") ?? "10"), page: parseInt(searchParam.get("page") ?? "1") }));
    ...
    const toggleModalDelete = (item: Bookcase) => {
        const modelElement = document.getElementById("modal-bookshop") as HTMLElement;
        const modal = new Modal(modelElement);
    ...
    const handleDelete = () => {
        if (categoryDelete) dispatch(deleteBookcase(categoryDelete?._id))
    }
    return (
        <>
            {loading ?
                ...
                <div>
                    ...
                    <div className="box-handle mb-3">
                        <Link to="/bookcases/create" className="btn-fill rounded">Create</Link>
                                            ...
                                            <tr key={item._id}>
                                                ...
                                                <td className="align-middle">
                                                    <div className="btn-group d-flex gap-2 align-items-center">
                                                        <Link className="btn-icon" to={`/bookcases/view/${item._id}`} data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="View">
                                                            <div className="icon">
                                                                <RiInformation2Line />
                                                            </div>
                                                        </Link>
                                                        <Link className="btn-icon" to={`/bookcases/edit/${item._id}`} data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Edit">
                                                            <div className="icon">
                                                                <RiPencilLine />
                                                            </div>
                                                        </Link>
