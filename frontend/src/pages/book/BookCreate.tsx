...
import { createBook } from "../../apis/actions/book.action";
...
const BookCreate = () => {
  ...
  const { categories: subcategories, loadingCategory, success, error, message } = useAppSelector((state) => state.book);
  ...
  const { handleSubmit, control, formState: { errors }, getValues } = useForm<Book>({
    defaultValues: {
      categories: []
    }
  });
